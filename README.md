<p align="center">
  <img src="Screenshot 2025-11-11 220655.png" 
    alt="Tushar P Banner" width="720" />
</p>

<h1 align="center">Tushar P</h1>
<p align="center" style="font-size:1.3em; color:#b0b0ff;"><b>Innovating the Future • Byte by Byte</b></p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=tung-programming&style=flat-square&color=6f42c1" alt="Profile views"/>
</p>

---

<blockquote align="center">
<em>“Code is like humor. When you have to explain it, it’s bad.” — Cory House</em>
</blockquote>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&pause=950&color=00EFFF&width=435&lines=Exploring+ML+%7C+IoT+%7C+EdTech;Open+Source+Builder;Collab%2C+Hackathons%2C+and+Beyond!"/>
</p>

## 🚩 Flagship Projects

<table>
<tr>
<td width="48%" valign="top">
  <h3>🧠 <a href="https://github.com/tung-programming/PolyLingo">PolyLingo</a></h3>
  <b>Multilingual AI Voice Chatbot</b><br>
  <img src="https://img.shields.io/badge/React-blue?logo=react" /> <img src="https://img.shields.io/badge/FastAPI-lightgrey?logo=fastapi" /> <img src="https://img.shields.io/badge/Groq-8B%20LLaMA-critical?logo=python" />
  <br><br>
  PolyLingo is an advanced AI chatbot supporting real-time voice & text in multiple languages, emotional awareness, and persona control. It blends NLP, Speech-to-Text, Text-to-Speech, and Emotion AI for truly human-like conversations, complete with a glassmorphic UI.
  <ul>
    <li>🔤 Multilingual speech & persona-based interaction</li>
    <li>😊 Emotion detection and response adaptation</li>
    <li>🎤 Voice input & dynamic feedback</li>
    <li>💡 Contextual replies powered by LLaMA (Groq)</li>
    <li>🎯 XP gamification and level system</li>
  </ul>
</td>
<td width="4%"></td>
<td width="48%" valign="top">
  <h3>🥕 <a href="https://github.com/tung-programming/nutrigo">NutriGo</a></h3>
  <b>Food Scanner & Health Score App</b><br>
  <img src="https://img.shields.io/badge/Next.js-black?logo=nextdotjs" /> <img src="https://img.shields.io/badge/Supabase-green?logo=supabase" /> <img src="https://img.shields.io/badge/Gemini%20AI-orange?logo=google" />
  <br><br>
  NutriGo empowers users to scan foods, analyze ingredients, and receive AI-driven health scores and warnings. It integrates barcode scanning, Supabase DB, Google Gemini AI, and instant nutrition analytics.
  <ul>
    <li>📱 Mobile-ready barcode scanning</li>
    <li>🌱 AI-calculated health & nutrition scores</li>
    <li>📊 Tracking and logging for healthy eating</li>
    <li>🔐 Secure backend, robust error handling</li>
    <li>🟩 Supabase-powered data storage</li>
  </ul>
</td>
</tr>
<tr>
<td width="48%" valign="top">
  <h3>🛡️ <a href="https://github.com/tung-programming/BlockquestWeek1">PhishBlock</a></h3>
  <b>Smart Phishing Intelligence Dashboard</b><br>
  <img src="https://img.shields.io/badge/Firebase-yellow?logo=firebase" /> <img src="https://img.shields.io/badge/React-blue?logo=react" /> <img src="https://img.shields.io/badge/Polygon-9000ff?logo=polygon" />
  <br><br>
  PhishBlock is an intelligence and archival system for phishing detection. It allows reporting, verifies domains, logs evidence on-chain, and archives via IPFS, featuring fully responsive feeds and blockchain anchors.
  <ul>
    <li>🔗 Real-time phishing verification</li>
    <li>🚀 Blockchain-based proof & archiving</li>
    <li>🌐 Secure Firebase backend</li>
    <li>💾 Firestore live feeds</li>
    <li>📱 Mobile-first UI</li>
  </ul>
</td>
<td width="4%"></td>
<td width="48%" valign="top">
  <h3>📝 <a href="https://github.com/tung-programming/Clone-Fest-Codex-Frieza-Week1">Chyrp Lite Revamp</a></h3>
  <b>Modern Serverless Blogging Platform</b><br>
  <img src="https://img.shields.io/badge/Next.js-blue?logo=nextdotjs" /> <img src="https://img.shields.io/badge/Firebase-yellow?logo=firebase" /> <img src="https://img.shields.io/badge/TailwindCSS-0af?logo=tailwindcss" />
  <br><br>
  This project transforms a legacy PHP platform into a modern app with Next.js & Firebase. It features authentication, rich content types, modular themes, role management, and SEO optimization.
  <ul>
    <li>✏️ Rich blog post types (Text, Photo, Quote...)</li>
    <li>🎨 Dynamic themes & SSR performance</li>
    <li>👥 User roles & interactive features</li>
    <li>👀 SEO + Analytics tracking</li>
    <li>🚦 Spam/captcha defense</li>
  </ul>
</td>
</tr>
</table>

---

## 🛠️ Tech Arsenal

<p align="center">
<img src="https://skillicons.dev/icons?i=python,js,typescript,react,fastapi,firebase,html,nextjs,docker,git,tailwindcss,supabase,polygon" />
</p>

---

## 🌟 Achievements & Badges

<p align="center">
  <img src="https://img.shields.io/badge/Open%20Source-14%2B%20repos-blue"/>
  <img src="https://img.shields.io/badge/Hackathon%20Participant-Yes-violet"/>
  <img src="https://img.shields.io/badge/RVCE%20Presenter-Technical-green"/>
  <img src="https://img.shields.io/badge/Workshop-Hands%20on-important"/>
  <img src="https://img.shields.io/badge/AI%2FML%20Innovation-On%20fire-orange"/>
</p>

---

## 📊 Contributions & Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=tung-programming&show_icons=true&theme=radical"/>
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=tung-programming&theme=dark"/>
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=tung-programming&theme=dark"/>
</p>

---

## 👑 Let's Collaborate

<div align="center">
  <a href="https://www.linkedin.com/in/tushar-p/">
    <img src="https://img.shields.io/badge/LinkedIn-blue?logo=linkedin"/>
  </a>
  <a href="mailto:tushar.p@email.com">
    <img src="https://img.shields.io/badge/Email-Contact%20Me-c14438?logo=gmail"/>
  </a>
  <a href="https://github.com/tung-programming">
    <img src="https://img.shields.io/badge/GitHub-Follow-black?logo=github"/>
  </a>
</div>

---

<p align="center"><em>“Make it majestic. Make it yours.”</em></p>
